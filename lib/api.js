...
function transpile(sourcePath, options = {}) {
    ...
    const coreSCSS = !optionsWithDefaults.isStandalone
    ? fs.readFileSync(path.join(__dirname, "../dist/flecss.scss")).toString()
    : "";
    ...
    const compile = (scss) => {
        ...
        return {
            ...
            ? new CleanCSS({}).minify(sassResult.css).styles
            : sassResult.css,
            loadedUrls: sassResult.loadedUrls
        };
    };

    const coreResult = compile(coreSCSS);
    const sourceResult = compile(sourceSCSS);
    return {
        coreCSS: coreResult.css,
        sourceCSS: sourceResult.css,
        loadedUrls: [ ...coreResult.loadedUrls, ...sourceResult.loadedUrls ]
    }
}
