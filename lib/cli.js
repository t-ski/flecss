...
class Args {
    ...
    static parseOption(key, shorthand) {
    	const index = Args.#retrieveKeyIndex(key, shorthand);
    	return (index >= 0 && ++index < process.argv.slice(2).length)
        ? {
...
const IS_WATCH = Args.parseFlag("watch", "W");
...
async function build() {
    ...
    try {
        result = await buildCSS(Args.parsePositional(0) ?? "", Args.parsePositional(1) ?? "", {
            isDevelopment: IS_WATCH,
            isStandalone: Args.parseFlag("standalone", "S"),
            modTimeTolerance: (!iterations || lastError) ? Infinity : WATCH_INTERVAL
        });
    } catch(err) {
        (err.toString() !== lastError)
        && console.error(`\x1b[31m${err}\x1b[0m`);

        !IS_WATCH && process.exit(1);

        lastError = err.toString();
    }

    IS_WATCH
    && setTimeout(build, WATCH_INTERVAL);
    ...
    console.log(`${
        ...
        .join(":")
    }\x1b[22m \x1b[38;2;${[ 251, 111, 177 ].join(";")}m${
        `\x1b[1m${_config.appName}\x1b[22m successfully ${IS_WATCH ? "watch" : "\x1b[1D"} built \x1b[3m${path.basename(result.targetPath)}\x1b[23m`
    }\x1b[2m\x1b[39m (${
