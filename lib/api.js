...
function transpile(sourcePath, isDevelopment = false, isStandalone = false) {
    const sourceSCSS = fs.readFileSync(sourcePath).toString();
    const sassResult = sass.compileString([
        !isStandalone ? CORE_SCSS : "",
        sourceSCSS
    ].join("\n"), {
    ...
    return {
        css: !isDevelopment ? new CleanCSS({}).minify(sassResult.css).styles : sassResult.css,
...
module.exports.buildCSS = function(sourcePath, targetPath, options) {
    ...
    const targetFilePath = targetPath.replace(/\/$/, `/${path.basename(sourceFilePath)}`).replace(/(\.s?css)?$/i, ".css");
    ...
    return new Promise((resolve, reject) => {
        ...
        const render = () => {
            const css = transpile(sourceFilePath, optionsWithDefaults.isDevelopment, optionsWithDefaults.isStandalone).css;
