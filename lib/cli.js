...
async function watch(watchInterval = WATCH_INTERVAL) {
    ...
    try {
        result = await build(Args.parsePositional(0) ?? "", Args.parsePositional(1), {
            ...
            ibrary: Args.parseOption("library", "L").string
        });
    } catch(err) {
    ...
    if(IS_DEVELOPMENT) {
        const timeout = Math.min(result.transpilerOutput ? WATCH_INTERVAL : watchInterval**1.25, 2.5 * WATCH_INTERVAL);
    ...
    console.log(`${
        (!lastError && iterations) ? "\x1b[2K\r\x1b[1A\x1b[2K\r\x1b[1A\x1b[2K\r" : ""
